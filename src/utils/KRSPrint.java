...
public class KRSPrint implements Printable {
    
    private JTable table;
    private static final int MARGIN = 50;
    
    public KRSPrint(JTable table) {
        this.table = table;
    }
    ...
    public int print(Graphics graphics, PageFormat pageFormat, int pageIndex) throws PrinterException {
        ...
        int[] columnWidths = {80, 100, 100, 150, 80}; // ID KRS, ID Mahasiswa, Kode MK, Nama MK, SKS
        int startY = 100;
        int startX = MARGIN;

        // Draw table headers
        String[] headers = {"ID KRS", "ID MHS", "KODE MK", "NAMA MK", "SKS"};
        int currentX = startX;
        ...
        for (int row = 0; row < table.getRowCount(); row++) {
            ...
            for (int col = 0; col < table.getColumnCount(); col++) {
                ...
                // Add SKS to total
                if (col == 4) { // SKS column
                    totalSKS += Integer.parseInt(text);
                }
            }
            currentY += rowHeight;
        }
        ...
        // Draw total SKS
        g2d.setFont(new Font("Arial", Font.BOLD, 12));
        String totalText = String.format("Total SKS: %d", totalSKS);
